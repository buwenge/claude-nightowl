# claude-nightowl 使用手册（中文）

> 这是逐项的操作说明；项目介绍看 [README.zh-CN.md](../README.zh-CN.md)（English: [README.md](../README.md)）。包名与命令叫 `nightshift`（夜班）——让 Claude Code 在你睡觉时按计划在 tmux 里干活。
...
> **用一年期令牌时**（`claude setup-token` 生成、放在 `CLAUDE_CODE_OAUTH_TOKEN`）：这种令牌只能发模型
> 请求，`/usage` 在它底下什么都不打印。调度器改读 `runners.claude.rate_limits_file`（默认
> `~/.claude/rate_limits.json`），零额外请求。这个文件由 Claude Code 的 statusLine 脚本维护——每个
> 会话每次回复都会收到 `rate_limits`，脚本把它合并写进去（示例脚本 `docs/statusline_rate_limits.py`）：
>
> ```json
> {"updated_at": 1788534845.4, "source": "statusline", "model": "claude-fable-5-1",
>  "windows": {"five_hour": {"utilization": 23.0, "resets_at": "2026-09-04T18:10:00+00:00", "at": 1788534845.4},
>              "seven_day": {"utilization": 17.0, "resets_at": "2026-09-09T12:00:00+00:00", "at": 1788534845.4}}}
> ```
>
> `utilization` 是已用百分数；刷新时刻已过的窗口按 0% 算。单模型周线（`Current week (Fable)`）状态栏
> 拿不到，只有跑该模型的 stream-json 子进程的 `rate_limit_event` 才带（键 `seven_day_overage_included`），
> 键到标签的映射在 `runners.claude.scoped_window_labels`（默认 `{"seven_day_overage_included": "Fable"}`）。
> 文件还没有时预检报"共享水位文件还没有"，随便开一个 `claude` 窗口回一句就有了。
>
> 自己写别的程序往这个文件合并读数时，"读出→合并→写回"要跟状态栏排队：锁同一个
> `~/.claude/.rate_limits.json.lock`（`fcntl.flock` 独占），写用临时文件+`os.replace`。
> 不排队的话好几个窗口同时刷新，后写的会冲掉先写的。夜班自己只读不写。
...
**交接文件怎么写。** 就是一个普通 markdown，路径开班提示词末尾就给了、到线提醒里再给一遍
（`tasks/<任务id>/handover-<班次>.md`；Codex 班在 `tasks/<任务id>/background/handover-<班次>.md`，沙箱只放开了这个子目录）；仓库之外还要写别的目录（比如某个项目的生产数据目录）就在 `runners.codex.extra_writable_roots` 里按项目声明，如 `{"myproject": ["/data/myproject/db"], "*": []}`，`*` 对所有项目生效——起班时一并放开，提示词里也会把清单告诉模型，免得它撞上只读就停下来等人）。把"已完成 / 未完成 / 下一步"写清楚，
最后一行必须是调度器认的指令：
...
例外：最终回复末行写了 `NEXT: continue` / `NEXT: done`、却没把交接落成文件的
（9/11 之前开班提示词不给路径，没撞线就收工的班常这样），hook 会在 Stop 时把这条
回复原文存成交接文件，之后照常按末行 NEXT 换班或完结；事件里记一条
"交接只写在最终回复里 → 已替它存为 handover-N.md"。只补不盖：模型自己写过的
交接文件原样保留。
