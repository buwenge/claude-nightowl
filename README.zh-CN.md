...
- **带预检的定时。** 任务可以定在某个时刻跑，也可以定在另一个任务结束之后跑。起跑前调度器检查：目标目录已被 Claude Code 信任、账号额度在你设的线以上——不够就**推迟**（默认 30 分钟一步，最多 6 小时），绝不硬起。
...
- **额度守卫。** 每 5 分钟解析一次 `claude -p /usage`（本地斜杠命令，不耗额度）。用一年期令牌（`claude setup-token` 放进 `CLAUDE_CODE_OAUTH_TOKEN`）时 `/usage` 是空的，守卫改读一个共享文件（`runners.claude.rate_limits_file`，默认 `~/.claude/rate_limits.json`）：每个 Claude Code 会话每次回复都会收到 `rate_limits`，让状态栏脚本顺手写进去。零额外请求，谁在花额度谁就在刷新数字。文件格式见 `docs/使用手册.md`。撞五小时线时会话自己设闹钟、刷新后接着干；撞周线就收尾。子 agent 也会收到一句短提醒，不会在主会话睡下后继续烧。
