...
# 静默 55 分钟兜底（闹钟大概率丢了）两处会真的 send-keys 这句。
DEFAULT_CLAUDE_RESUME_TEXT = (
    "来自nightshift：五小时额度应已刷新，请从刚才停下的地方继续。"
)
# F3：Claude 没有调度器能感知的"闹钟到底响没响"信号——ScheduleWakeup 若被
# CC 的 cron 丢了（没触发），waiting_wakeup 会永远等下去。兜底：额度刷新
# 时间已过、且会话静默（距最后一个 hook 事件）满这么多分钟仍没自己醒，
# 调度器主动 send-keys 叫它继续，跟 idle 分支走同一套文案/失败处理。
# 按"静默时长"而不是"刷新后过了多久"算，且取 55：CC 的缓存 TTL 是 60 分钟，
# 敲进去时缓存还得是热的（工头 9/2 拍板）；Codex 不走这条——它到点就由
# 上面的 codex_waiting_wakeup 分支直接敲。
CLAUDE_WAKEUP_GRACE_MINUTES = 55
...
def _check_running(
    ...
    if (
        ...
        if status.get("state") == "waiting_wakeup":
            ...
            # 刷新时间已过（上面已判）且会话静默满 CLAUDE_WAKEUP_GRACE_MINUTES
            # 仍没等到它自己醒，就由调度器主动 send-keys 叫它继续，跟上面
            # idle 分支同一套文案/失败处理。静默按最后一个 hook 事件算
            # （waiting_wakeup 期间没有事件，last_event_at 就是进入等待的那次
            # Stop 或上一次闹钟醒来的 Stop），没有就退回按刷新时刻算。
            # 没到静默线（或本来就是 Codex，已在上面 codex_waiting_wakeup
            # 分支处理过）仍然只能等。
            silent_since = parse_iso(status.get("last_event_at") or paused_until)
            if (
                runner == "claude"
                and not status.get("quota_resume_sent")
                ...
                store.append_event(
                    task_id,
                    f"额度刷新已过、会话静默超过 {CLAUDE_WAKEUP_GRACE_MINUTES} 分钟仍未自醒，"
                    "已 send-keys 让它继续",
                )
                return [f"{task_id} 额度刷新已过且静默 {CLAUDE_WAKEUP_GRACE_MINUTES} 分钟未自醒，敲它继续"]
            return []  # Claude：闹钟还没响完/静默还没到线，等它自己醒
    ...
    if status.get("state") == "idle":
        if store.role_of(task) == "review":
            ...
            if result is not None:
                return result
        elif _handover_needs_eval(task, status):
...
def _handover_mtime_ns(task: dict) -> int | None:
    """交接文件当前的 mtime_ns；文件不存在返回 None。"""
    try:
        return _handover_file(task).stat().st_mtime_ns
    except OSError:
        return None


def _handover_needs_eval(task: dict, status: dict) -> bool:
    """H6：这一班的交接要不要（重新）评估换班。

    以前 `chain_checked` 是一次性布尔——评估过一次，之后哪怕交接文件被
    人工在同一窗口重写（NEXT: done 停下后又聊了几个小时、把末行改成
    continue），也永远不会再被摸到（9/2 真机事故）。改成认交接文件的
    mtime：没评估过、或评估过之后文件又被写过一次，都要（重新）评估。

    `successor_id` 已经有值时强制 False——这班已经有后继了，重写交接
    也不许再造第二个后继（人工在已换班的父窗口里聊天不该开出并行的第
    三班）；这种情形留给 H8 把状态摁回 chained。
    """
    if not status.get("chain_checked"):
        return True
    if status.get("successor_id"):
        return False
    mtime_ns = _handover_mtime_ns(task)
