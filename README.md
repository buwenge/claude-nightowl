...
- **Scheduling with preflight.** A task runs at a time, or after another task ends. Before launching, the scheduler checks the target directory is trusted by Claude Code and that your account quota is above the configured lines — and *postpones* (default 30 min, up to 6 h) instead of launching into a wall.
